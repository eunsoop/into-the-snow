...
class Furnace(Entity):
    ...
    def paint(self, surface: pygame.Surface):
        """
        Draw the furnace block, its door, and a animated fire glow if fuel > 0.
...
class CraftingTable(Entity):
    ...
    def paint(self, surface: pygame.Surface):
        """
        Draw the wooden crafting table and blueprint icon lines.
        :param surface: Target drawing Surface
        """
...
class BrokenEngineCore(Entity):
    ...
    def __init__(self, x: float, y: float):
        """
        Initialize the BrokenEngineCore.
        :param x: World X coordinate
        :param y: World Y coordinate
        """
        super().__init__(x, y)
