...
class Player(Entity):
    ...
    def remove_item(self, item: str, amount: int = 1) -> bool:
        """
        Deduct a specific amount of an item if sufficient count exists.
        :param item: Item type string
        :param amount: Quantity to remove
        :return: True if successfully removed, False if player had insufficient items
        """
    ...
    def get_item_count(self, item: str) -> int:
        ...
        :param item: Item type string to look up
        :return: Integer count of the item
        """
        return self.inventory.get(item, 0)

    def has_item(self, item: str) -> bool:
        """
        Check if the player has at least one instance of a specific item.
        :param item: Item type string
    ...
    def pop_transition_x(self) -> float | None:
        """
        Retrieve and clear the pending horizontal scene transition position.
        :return: The target transition coordinate or None
        """
    ...
    def pop_transition_y(self) -> float | None:
        """
        Retrieve and clear the pending vertical scene transition position.
        :return: The target transition coordinate or None
        """
    ...
    def pop_spotted_shake(self) -> bool:
        """
        Determine if the player was newly spotted by a guard to trigger alert camera shake.
        :return: True if newly spotted, clearing flag afterwards
        """
    ...
    def update(self):
        if self.layer and self.layer.get_game():
            ...
            if is_warming: self.temperature = min(100.0, self.temperature + 12.0 * dt)
            else: self.temperature -= .5 * dt
